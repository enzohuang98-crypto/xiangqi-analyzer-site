...
// dist/ is generated by build-dist.mjs from these same sources; scanning it
// would double-report every finding against a copy nobody edits.
...
// A page's <meta> CSP and the _headers CSP are both enforced, and the browser
// applies their intersection. Drift between them tightens the policy with no
// error anywhere — the usual symptom is stylesheets silently failing to load.
function checkCspConsistency() {
  ...
  if (!headerLine) {
    errors.push('_headers: 缺少 Content-Security-Policy 標頭')
    return
  }

  const headerCsp = parseCsp(headerLine.replace(/^\s*Content-Security-Policy:/i, ''))
  ...
  for (const file of htmlFiles) {
    ...
    for (const [name, directive] of parseCsp(match[1])) {
      if (!headerCsp.has(name)) {
        fail(file, `CSP 指令 "${name}" 未出現在 _headers，兩者取交集會不一致`)
      } else if (headerCsp.get(name) !== directive) {
        fail(
          file,
          `CSP 指令 "${name}" 與 _headers 不一致：\n` +
            `    meta:     ${directive}\n` +
            `    _headers: ${headerCsp.get(name)}`
        )
      }
    }
  }
